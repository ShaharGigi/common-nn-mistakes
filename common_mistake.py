...
def main():
    ...
    with missinglink_project.create_experiment(
        ...
        for epoch in experiment.epoch_loop(args.epochs):
            train(model, optimizer, epoch, [first_batch] * 50, [first_batch]) # First batch
            # train(model, optimizer, epoch, [first_batch] * 50, [next(iter(test_loader))]) # Overfitting first_batch*50 and first batch test_loader
            # train(model, optimizer, epoch, [first_batch] * 50, test_loader)   # Overfitting first_batch*50 and full test_loader
            # train(model, optimizer, epoch, train_loader, test_loader)   # Full train-test
